...
if [ -d "build/macroknob_dongle" ] && [ -d "build/macroknob_lily58_left" ] && [ -d "build/macroknob_lily58_right" ] && [ -d "build/settings_reset" ]; then
    ...
    west build -p -d build/macroknob_lily58_left -b nice_nano_v2 -S studio-rpc-usb-uart -- -DSHIELD=macroknob_lily58_left -DZMK_EXTRA_MODULES=$work_dir -DCONFIG_ZMK_STUDIO=y
    west build -p -d build/macroknob_lily58_right -b nice_nano_v2 -S studio-rpc-usb-uart -- -DSHIELD=macroknob_lily58_right -DZMK_EXTRA_MODULES=$work_dir -DCONFIG_ZMK_STUDIO=y
